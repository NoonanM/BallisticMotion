...
for(G in 1:GENS){
  ...
  for(R in 1:REPS){
    ...
    if(G == 1){
      ...
      for(i in 1:n_pred){
        ...
        pred_tau_p <- pred.tau_p(mass_pred)
        pred_tau_v <- pred.tau_v(mass_pred)
  ...
  print(G)
  save(prey_res, file = 'Results/lv_Evo_Prey_Cost_Ben.Rda')
  save(prey_details, file = 'Results/lv_Evo_Prey_Cost_Ben_details.Rda')
}

#Compile the results
load('Results/lv_Evo_Prey_Cost_Ben.Rda')
load('Results/lv_Evo_Prey_Cost_Ben_details.Rda')
